...
public class SelectingSuggestHBox extends HBox {
	...
	public void generateSuggestions() {
		...
		while (startIndex >= 1 && ScoutingApp.mStage.getAllEvents()[startIndex - 1].start_date
				.compareTo(ScoutingApp.mStage.getAllEvents()[startIndex].start_date) == 0)
			startIndex--;

		// System.out.println("TEAM FOUND: " +
		// ScoutingApp.mStage.getAllEvents()[startIndex].name);

		suggested = new Event[Math.min(n, ScoutingApp.mStage.getAllEvents().length - startIndex)];
		int index = 0;
		for (int i = startIndex; i < Math.min(startIndex+n, ScoutingApp.mStage.getAllEvents().length); i++) {
			suggested[index] = ScoutingApp.mStage.getAllEvents()[i];
			index++;
		}
		for (Event e : suggested) {
			if (e != null) {
				SuggestedLabel l = new SuggestedLabel(e);
				l.setOnMouseClicked(new onLabelClicked(e));
				labels.add(l);
				contents.getChildren().addAll(l);
			}
		}

		contents.setPadding(new Insets(BoxPaddingInsets.OFFSET / 2.0, BoxPaddingInsets.OFFSET / 2.0,
				BoxPaddingInsets.OFFSET * 7.0 / 4, BoxPaddingInsets.OFFSET / 2.0));
...
class SuggestedLabel extends Label {
	...
	public SuggestedLabel(Event event) {
		...
		this.setPadding(new Insets(BoxPaddingInsets.OFFSET / 2.0, BoxPaddingInsets.OFFSET,
				BoxPaddingInsets.OFFSET / 2.0, BoxPaddingInsets.OFFSET));
	}
